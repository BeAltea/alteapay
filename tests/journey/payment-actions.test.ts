...
function reset(paymentOrigin = "platform") {
  db = {
    tenant_chat_config: [{ company_id: CO, payment_origin: paymentOrigin }],
    agreements: [],
...
describe("paymentCreate (papel A)", () => {
  ...
  it("payment_origin=n8n desabilita payment.create", async () => {
    reset("n8n")
    const { paymentCreate } = await import("@/lib/journey/payment-actions")
    const r = await paymentCreate(ctx, "o1")
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.code).toBe("payment_origin_n8n")
  })
})
